...
fn shadow_mode_dedicated_rules_omit_mapping() {
    ...
    assert!(
        rules.contains("Surgical precision"),
        "shadow must include principles"
    );
    assert!(
        rules.contains("Parallelism"),
        "shadow must include parallelism"
    );
    assert!(
        rules.contains("Token discipline"),
        "shadow must include token discipline"
    );
    assert!(
        rules.contains("Understanding first"),
        "shadow must include understanding-first"
    );
    assert!(
    ...
    assert!(
        !rules.contains("<!--"),
        "shadow must not include HTML comments"
    );
}
...
fn shadow_mode_shared_rules_omit_mapping() {
    ...
    assert!(
        rules.contains("Surgical precision"),
        "shadow shared must include principles"
    );
    assert!(
        !rules.contains("Tool Mapping"),
        "shadow shared must not include mapping header"
    );
    assert!(
        !rules.contains("MUST USE"),
        "shadow shared must not include tool mapping"
    );
    assert!(
        !rules.contains("<!--"),
        "shadow shared must not include HTML comments"
    );
}
