...
pub(crate) fn strip_lean_ctx_blocks(content: &str) -> String {
    ...
    loop {
        let next = if out.contains(BLOCK_START) && out.contains(BLOCK_END) {
            ...
            crate::marked_block::remove_content(&out, COMPRESSION_BLOCK_START, COMPRESSION_BLOCK_END)
        } else {
            break;
        };
        if next == out {
...
pub(crate) fn strip_compression_block(content: &str) -> String {
    ...
    let stripped = crate::marked_block::remove_content(content, COMPRESSION_BLOCK_START, COMPRESSION_BLOCK_END);
